    ...
    <div class="row">
        <div class="col-12">
            <div class="card card-new-styles">
                ...
                <div class="card-body">
                    <p class="custom_domain_para text-black">{{get_static_option_central('custom_domain_settings_description')}}</p>
                                            ...
                                            @foreach($user_domain_infos->tenant_details ?? [] as $tenant)
                                                ...
                                                <td class="py-4">
                                                    @if(optional($tenant->custom_domain)->custom_domain_status == 'pending')
                                                        <span class="alert alert-warning text-capitalize">{{__(optional($tenant->custom_domain)->custom_domain_status)}}</span>
                                                    @elseif(optional($tenant->custom_domain)->custom_domain_status == 'in_progress')
                                                        <span class="alert alert-info text-capitalize">{{__(str_replace('_',' ',optional($tenant->custom_domain)->custom_domain_status))}}</span>
                                                    @elseif(optional($tenant->custom_domain)->custom_domain_status == 'connected')
                                                        <span class="alert alert-success text-capitalize">{{__(optional($tenant->custom_domain)->custom_domain_status)}}</span>
                                                    @elseif(optional($tenant->custom_domain)->custom_domain_status == 'rejected')
                                                        <span class="alert alert-info text-capitalize">{{__(str_replace('_', ' ',ucwords(optional($tenant->custom_domain)->custom_domain_status)))}}</span>
                                                    @elseif(optional($tenant->custom_domain)->custom_domain_status == null)

                                                    @else
                                                        <span class="alert alert-danger text-capitalize">{{__(optional($tenant->custom_domain)->custom_domain_status) ?? __('Removed')}}</span>
                                                    @endif
                                                </td>
                                                <td>
