            ...
            <div class="card">
                <div class="card-body">
                    <x-admin.header-wrapper>
                        <x-slot name="left">
                            <h4 class="card-title">{{__('Current Package Information')}} {{__('(from main site)')}}</h4>
                        </x-slot>
                    </x-admin.header-wrapper>

                    <ul>
                        <li><strong>{{__('Package Name')}}:</strong> {{$current_package->package_name}}</li>
                        <li><strong>{{__('Package Price')}}:</strong> {{amount_with_currency_symbol($current_package->package_price)}}</li>
                        <li><strong>{{__('Expire Date')}}:</strong> {{ tenant() && tenant()->expire_date ? tenant()->expire_date->format('d M Y') : '' }} -
                            {{tenant() && tenant()->expire_date ? tenant()->expire_date->diffForHumans(parts: 4) : '' }}</li>
                    </ul>
                    ...
                    <div class="d-flex justify-content-start gap-5">
                        ...
                        <div>
                            <p><Strong>{{__('Features')}}</Strong></p>
                            <ol>
                                @foreach($features ?? [] as $feature)
                                    <li class="text-capitalize">{{str_replace('_', ' ', $feature->feature_name)}}</li>
                                @endforeach
                            </ol>
                        </div>

                        <div>
                            <p><Strong>{{__('Payment Gateways')}}</Strong></p>
                            <ol>
                                @foreach($payment_gateways ?? [] as $feature)
                                    <li class="text-capitalize">{{str_replace('_',' ',$feature->payment_gateway_name)}}</li>
                                @endforeach
                            </ol>
                        </div>

                        <div>
                            <p><Strong>{{__('Themes')}}</Strong></p>
                            <ol>
                                @foreach($themes ?? [] as $feature)
                                    <li class="text-capitalize">{{str_replace('_',' ', $feature->theme_slug)}}</li>
                                @endforeach
                            </ol>
                        </div>
                    </div>
