...
class ModuleMetaData
{
    ...
    public function getAllPaymentGatewayListWithImage(bool $is_price_plan)
    {
        $eachIndex = 0;
        $outputMarkup = [];
        $allMetaInformation = $this->getAllMetaData();
        foreach ($allMetaInformation as $index => $metaInfo) {
            ...
            if (!empty($paymentMeta)) {
                foreach ($paymentMeta as $key => $meta) {
                    ...
                    if (! $is_price_plan) {
                        if (! tenant() && property_exists($meta, 'landlord') && $meta->landlord === false) {
                            continue;
                        }
                    }
    ...
    public function getAllTenantPaymentGatewayListWithImage($plan_based_payment_gateway)
    {
        $eachIndex = 0;
        $outputMarkup = [];
        $allMetaInformation = $this->getAllMetaData();
        foreach ($allMetaInformation as $index => $metaInfo) {
            ...
            if (!empty($paymentMeta)) {
                foreach ($paymentMeta as $key => $meta) {
                    if (! in_array($meta->slug, $plan_based_payment_gateway))
                    {
                        continue;
                    }
                    ...
                    if (tenant())
                    {
                        $status = get_static_option("{$meta->slug}_status"); // for tenant based on the status they set from admin panel
                        if (property_exists($meta, 'tenant') && !$meta->tenant || empty($status)) {
                            continue;
                        }
                    }

                    if (!tenant() && property_exists($meta, 'landlord') && !$meta->landlord)
                    {
                        continue;
                    }
    ...
    private function getEachPaymentMenu($allModuleMeta)
    {
        ...
        if (!empty($allModuleMeta)) {
            foreach ($allModuleMeta ?? [] as $metaData) {
                ...
                if (property_exists($metaData, "status") && ! $metaData->status) {
                    continue;
                }
        ...
        if (!empty($allModuleMeta)) {
            foreach ($allModuleMeta ?? [] as $metaData) {
                ...
                if (!empty((array)$menuItem)) {
                    if (tenant())
                    {
                        if (property_exists($metaData, "alias") && !empty($metaData->alias))
                        {
                            if (!in_array(str_replace(['-', '_'], '', $metaData->alias), $this->coreModuleList()))
                            {
                                if (!tenant_plan_sidebar_permission(str_replace(['-', '_'], '', $metaData->alias)))
                                {
                                    continue;
                                }
                            }
...
//                        {
//                            if (property_exists($item, "permissions") && !empty($item->permissions))
//                            {
//                                if (is_array($item->permissions))
//                                {
//                                    foreach ($item->permissions as $each_permission)
//                                    {
//                                        if (!in_array(str_replace(['-', '_'], '', $each_permission), $this->coreModuleList()))
//                                        {
//                                            if (!tenant_plan_sidebar_permission(str_replace(['-', '_'], '', $each_permission)))
//                                            {
//                                                $flag = true;
//                                                break;
//                                            }
...
//                                    }
//                                } else {
//                                    if (!in_array(str_replace(['-', '_'], '', $item->permissions), $this->coreModuleList()))
//                                    {
//                                        if (!tenant_plan_sidebar_permission(str_replace(['-', '_'], '', $item->permissions)))
//                                        {
//                                            $flag = true;
//                                        }
//                                    }
...
//                        }
    ...
    private function coreModuleList(): array
    {
        return [
            'integrations'
        ];
...
//                    $paymentGateway = $metaItem->paymentGateway;
//                    if (property_exists($paymentGateway, 'admin_settings')) {
//                        $admin_settings = $paymentGateway->admin_settings;
//
//                        if (tenant() && property_exists($admin_settings, 'show_admin_tenant') && $admin_settings->show_admin_tenant === false) {
//                            continue;
//                        }
//                    }
    ...
    public function getExternalPluginsName(): array
    {
        ...
        foreach ($externalPlugins ?? [] as $key => $plugin)
        {
            if (!empty($plugin))
            {
                ...
                $type = 'general';
                if(property_exists($plugin, 'nazmartMetaData') && !empty($plugin->nazmartMetaData))
                {
                    $nazmart_meta_data = $plugin->nazmartMetaData;
                    if (property_exists($nazmart_meta_data, 'paymentGateway'))
                    {
    ...
    public function getAllPluginsRouteList(string $routeType = 'route'): array
    {
        $routeList = $this->getAllExternalMenu();
        ...
        foreach ($routeList ?? [] as $list)
        {
            ...
            {
                if (property_exists($item, $routeType) && !empty($item->$routeType))
                {
                    $tenantRouteArr = explode('.', $item->$routeType);
                    if (in_array('admin', $tenantRouteArr))
                    {
                        $routeArray[$item->$routeType] = $item->label;
                    }
                }
            }
        }
    ...
    public function getTenantValidPluginsRouteList(array $exclude = []): array
    {
        $routeList = $this->getAllPluginsRouteList(routeType: 'tenantRoute');

        $final_list = [];
        foreach ($routeList as $index => $item)
        {
            $exploded_item = explode('.', $index);

            foreach ($exclude ?? [] as $exclude_item)
            {
                if (in_array($exclude_item, $exploded_item)) {
                    continue 2;
                }
            }
    ...
    public function getLandlordValidPluginsRouteList(array $exclude = []): array
    {
        $routeList = $this->getAllPluginsRouteList(routeType: 'route');
        ...
        foreach ($routeList as $index => $item)
        {
            ...
            if (in_array('tenant', $exploded_item))
            {
                continue;
            }
